...
### #55 — Tags AI en el response son frágiles — el AI no siempre los genera
**Error:** Los recordatorios dependían de que Claude generara un tag exacto `<<SELF_REMINDER:{"descripcion":"...","fecha":"...","hora":"..."}>>` en su respuesta. Claude a veces respondía conversacionalmente sin el tag, y el recordatorio nunca se creaba. El paciente recibía confirmación verbal pero nada se guardaba en la DB.
**Lección:** Para acciones críticas (crear recordatorio, hacer una reserva, cambiar un estado), NO depender de que el AI genere un formato exacto en texto libre. Usar un flujo determinístico pre-AI (state machine paso a paso, como el registro) que no puede fallar. Los tags AI son útiles para enriquecer respuestas, no para gatillar acciones que el usuario espera que sucedan siempre.

### #56 — Chat anónimo reutilizaba la conversación de un paciente por teléfono (fuga de salud)
**Error:** Al agregar el menú, un número NO registrado puede chatear con la IA (opción 1). `handleGeneralChat` armaba el historial con `getOrCreateConversation(phone, null)`, y esa función buscaba CUALQUIER conversación OPEN por teléfono, ignorando `patientId`. Como los números (sobre todo prepagos) se reasignan de dueño, una persona nueva en un número que fue de un paciente podía recibir, vía el historial que se le inyecta al LLM, datos de salud del dueño anterior (programas, controles, medicación). El cifrado en reposo no protege acá porque el sistema descifra y entrega el historial por diseño.
**Lección:** El estado/contexto "anónimo" NUNCA debe poder leer el de una entidad identificada que comparte la misma clave (teléfono, IP, sesión). Cuando una función de "get-or-create por clave" sirve tanto a flujos identificados como anónimos, el camino anónimo debe filtrar explícitamente por "sin dueño" (`patientId: null`). Regla general: aislar siempre lo anónimo de lo identificado aunque compartan el identificador de transporte.

### #57 — Camino anónimo al LLM sin tope por remitente = riesgo de costo/DoS
**Error:** Antes, un desconocido no llegaba al LLM sin registrarse. Con la opción 1 del menú, cualquier número dispara `generateResponse` por mensaje, sin límite por remitente — el único throttle era el rate-limit del webhook, que es un bucket GLOBAL (no por teléfono). Un número en loop podía inflar la factura de Anthropic y, de paso, agotar el bucket global y degradar el servicio para pacientes reales.
**Lección:** Cada vez que se abre un camino NO autenticado hacia un recurso pago (LLM, SMS, email), agregar un tope por remitente además del rate-limit global del transporte. El rate-limit del webhook protege la infra, no el presupuesto. Patrón usado acá: Map en memoria `phone → {count, ventana}` con TTL, y cota dura de tamaño (`boundedSet`) para que el propio anti-abuso no sea un vector de OOM.

### #58 — Una "marca para revisión" que ningún código respeta es decorativa
**Error:** En la fase 2, la autoinscripción por bot se guardaba con `reviewedAt=null` "para que el equipo la revise", pero el cron de recordatorios filtra por `status=ACTIVE` y NO mira `reviewedAt` → mandaba recordatorios de programas (incluso oncológicos) sin que nadie validara. El security-auditor lo marcó: la marca de revisión no protegía nada porque ningún flujo la consultaba, y además no había endpoint para setearla.
**Lección:** Cuando agregás un campo "pendiente de revisión / aprobación", decidí EXPLÍCITAMENTE una de dos: (a) algún flujo GATEA el comportamiento sensible sobre ese campo (el cron/acción ignora lo no-revisado), o (b) es solo un aviso informativo y lo documentás como tal. Lo que no podés es dejarlo en el limbo "parece un control pero no controla nada". Acá el dueño decidió (b) conscientemente (activa directo + badge de aviso); lo importante es que sea una decisión, no un descuido.

### #59 — Las escrituras del webhook se auditaban como SYSTEM, no como BOT
**Error:** El audit log (extensión de Prisma) toma el actor del AsyncLocalStorage. El middleware lo abre como SYSTEM y `requireAuth` lo eleva al médico. Pero el webhook de WhatsApp procesa en un IIFE fire-and-forget SIN elevar el actor → todas las escrituras del bot (registro, recordatorios, autoinscripción a programas) quedaban auditadas como SYSTEM, perdiendo la traza de que las originó el bot (el modelo AuditLog tiene actorType=BOT justo para esto).
**Lección:** Si tenés un actor de auditoría por canal (web autenticado, cron, bot/webhook), cada canal debe SETEAR su actor al entrar. Para el webhook: `setAuditActor({ actorType: 'BOT', actorId: null })` al inicio del procesamiento async. El default SYSTEM es un fallback, no una atribución correcta para acciones del bot.

### #60 — @unique solo dedupa strings idénticos: normalizá a forma canónica en TODA vía de escritura
**Error:** `Patient.dni` y `Patient.phone` tienen `@unique`, pero eso solo impide strings IDÉNTICOS. El teléfono se guardaba "como venía" según la vía: el bot lo recibe de Twilio como `+5493764125878` (con el 9 del móvil AR), mientras que panel/CSV aceptaban cualquier E.164 válido y guardaban lo tipeado, ej. `+543764125878` (sin el 9). Mismo número, dos strings distintos → el `@unique` NO lo detecta → mismo paciente duplicado. (El DNI estaba a salvo de casualidad: todas las vías terminan en dígitos puros.)
**Lección:** Una columna `@unique` solo garantiza unicidad si TODAS las vías de escritura normalizan a una ÚNICA forma canónica antes de buscar/guardar. Para identificadores con variantes de formato (teléfonos, emails, DNIs con puntos), centralizá un `canonicalX()` y aplicalo en cada path (bot, panel, import) — y al validar contra inyección, hacelo sobre el valor CRUDO antes de normalizar (ver [[lessons #53]]). El @unique es el backstop, no la única defensa. Para data ya existente hace falta un backfill que normalice + detecte colisiones (dos filas que canonicalizan al mismo valor = duplicado real a fusionar).
